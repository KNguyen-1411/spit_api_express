...
class NewsControllers {
  ...
  async index(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!news || news.length === 0) {
        res
          .status(404)
          .json({ data: "Không thành công!", message: message.NEWS_NOT_FOUND });
        return;
      }
      res.status(200).json({
        data: news,
  ...
  async create(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (existingNews) {
        res
          .status(400)
          .json({ data: "Không thành công!", message: message.NEWS_EXISTED });
        return;
      }
  ...
  async update(req: Request, res: Response): Promise<void> {
    const { id } = req.params;
    const { title, date, contentHeader, contentFooter, contentMain, image } =
      req.body;
