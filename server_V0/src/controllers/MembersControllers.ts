...
import message from '../messages/members.message';
class MembersControllers {
  ...
  async index(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!members || members.length === 0) {
        res.status(404).json({ data: 'Không thành công!', message: message.MEMBER_NOT_FOUND });
        return;
      }
      res.status(200).json({
        data: members,
  ...
  async create(req: Request, res: Response): Promise<void> {
    const { id, lastName, firstName, gender, birthday, email, phone, class: className, avatar, generation } = req.body;

    try {
      const existingMember = await Members.findOne({ where: { id } });
      if (existingMember) {
        res.status(400).json({ data: 'Không thành công!', message: message.MEMBER_EXISTED });
        return;
      }
  ...
  async update(req: Request, res: Response): Promise<void> {
    const { id } = req.params;
    const { lastName, firstName, gender, birthday, email, phone, class: className, avatar, generation } = req.body;
