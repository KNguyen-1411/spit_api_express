...
import message from '../messages/activities.message';
class ActivitiesControllers {
  ...
  async create(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (existingActi) {
        res.status(400).json({ data: 'Không thành công!', message: message.ACTIVITY_EXISTED });
        return;
      }
  ...
  async delete(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (result === 0) {
        res.status(404).json({ data: 'Không thành công!', message: message.ACTIVITY_NOT_FOUND });
        return;
      }
